...
class TypeDBClient(ConnectorPlugin):
    ...
    def _set_confidence(
        ...
        for thing in things:
            if isinstance(thing, (Entity, Relation)):
                cons = thing.get_attributes('confidence')
                if not cons:
                    ...
                    for con in cons:
                        if con.value == 0.0:
                            thing.has.remove(con)
                    _log.debug(f"Thing had {len(cons)} confidence levels.")
                    _log.debug(f"New confidence levels: {thing.get_attributes('confidence')}")
    ...
    def _set_user_uuid(
        ...
        """Sets '00000000-0000-0000-0000-000000000000' as user-uuid if Thing generated by LEDHNTR
        """
        _log = self.logger
        _log.debug(f"Adding system user-uuid to all Entities and Relations")
        default_user = Attribute(
            label='user-uuid',
        ...
        for thing in things:
            if isinstance(thing, (Entity, Relation)):
                users = thing.get_attributes('user-uuid')
                if not users:
                    thing.has.append(default_user)
                    counter += 1
                    continue
        _log.debug(f"Added {counter} user attributes.")
        return things
    ...
    def add_entity(
        ...
        entity = self._set_confidence([entity])[0]
        entity = self._set_user_uuid([entity])[0]
    ...
    def add_relation(
        ...
        relation = self._set_confidence([relation])[0]
        relation = self._set_user_uuid([relation])[0]
    ...
    def bulk_add_update(
        ...
        if new_things:
            ...
            new_things = self._set_user_uuid(new_things)
            tx = self.create_transaction(
                tx_type = TransactionType.WRITE,
                save_tx = False,
            )
